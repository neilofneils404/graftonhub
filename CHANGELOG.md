# Changelog

All notable changes to GraftonHub will be documented in this file.

## [1.0.0] - 2026-02-15

### Initial Launch 🎉

GraftonHub is live at [graftonhub.com](https://graftonhub.com)!

#### Features

**Core Pages (13 total)**
- Homepage with quick-nav cards and utility reference
- Emergency Services (by locality)
- Government (officials, meetings, contacts for all 5 localities)
- Schools & Education (3 districts + libraries + preschools)
- Utilities Setup Guide (organized by locality)
- Youth Sports (comprehensive directory with registration info)
- Parks & Trails (Metro Parks + local parks by locality)
- Events (annual festivals, farmers market, seasonal activities)
- New Residents Guide (complete first-week checklist)
- Services placeholder (future expansion)
- About page
- Custom 404 page (with Nancy's Diner easter egg)
- Nancy's Diner tribute page (hidden easter egg via footer ☕)

**Coverage**
- 5 localities: Grafton (Village), Oberlin (City), LaGrange (Village), Carlisle Township, Eaton Township
- 3 school districts: Midview, Keystone, Oberlin
- Youth sports: Baseball, softball, soccer, football, basketball, swimming, martial arts, dance
- Emergency services by locality with non-emergency contacts
- Complete utility setup guide by location
- 3 major Metro Parks + local parks
- Annual events and community resources

**Design**
- Clean, content-focused design with neutral colors
- Fully responsive (mobile, tablet, desktop)
- Complete dark mode support across all pages
- Accessible navigation and semantic HTML
- No tracking, no cookies, no analytics

**Infrastructure**
- Static site built with Astro
- Hosted on Cloudflare Pages (free tier)
- Custom domain: graftonhub.com
- Monthly verification maintenance cycle
- Git version control (local workspace + GitHub)

#### Easter Eggs
- ☕ Coffee cup in footer (hover animation) links to Nancy's Diner tribute page
- Custom 404 page with Nancy's Diner theme
- `/nancys` hidden page celebrating Grafton's legendary diner

#### Technical Details
- **Stack**: Astro (static site generator)
- **Hosting**: Cloudflare Pages
- **Domain**: graftonhub.com (~$12/year)
- **Maintenance**: Monthly verification (~$2-4/year)
- **Total annual cost**: ~$14-16

### Research & Verification

All information verified from official sources:
- Municipal and township websites
- School district websites
- Youth sports organization websites
- Lorain County government resources
- Local business websites and community sources

**Verification Date**: February 2026  
**Next Verification**: March 1, 2026

---

## Version History

- **v1.0.0** (2026-02-15): Initial public launch
  - Complete site with 13 pages
  - Full dark mode support (global.css dark mode CSS variables)
  - Nancy's Diner easter eggs
  - Monthly maintenance process established
  - Community validation: Positive feedback from Grafton Facebook group
